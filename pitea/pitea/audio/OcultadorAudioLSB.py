...
class OcultadorAudioLSB(OcultadorAudio):
    ...
    def ocultar(self, datos_imagen):
        ...
        assert self.audio.getsampwidth() == 2, (
            "El archivo de audio debe ser de 16 bits"
        )  # Asegurarse de que el archivo de audio sea de 16 bits

        frames = bytearray(
            list(self.audio.readframes(self.audio.getnframes()))
        )  # Leer los frames del archivo de audio

        if (
            len(binarios_imagen) > len(frames) * 8
        ):  # Asegurarse de que la imagen pueda ser ocultada en el archivo de audio
            raise ValueError(
                "La imagen es demasiado grande para ser ocultada en este archivo de audio."
            )
        ...
        for i in range(len(frames)):  # Iterar sobre los frames del archivo de audio
            if indice_datos < len(binarios_imagen):  # Si todavía hay datos para ocultar
                frames[i] = (frames[i] & 254) | int(
                    binarios_imagen[indice_datos]
